 ...
 * 0 button = 137
 *
 * This version of program just runs the motor in forward or reverse with pwm speed control
 * Up arrow is forward down arrow is reverse
 * middle button is stop
 * 0 - 9 buttons are speed control
...
const int enterButton = 229;  //middle button



// leg 5 (PB0) used for Software serial TX

#define CAM_SWITCH PIN_PB1  //NO switch tied to ground on leg 6 **** not used in this version
...
void motorForward()
{
  ...
  analogWrite(CONTROL_B, 255 - speed);  // lo pulses
  motor_state = 1;
}
void motorReverse()
{
  ...
  analogWrite(CONTROL_B,speed);   //high pulses
  motor_state = 2;
}
...
void setup() {
 ...
 digitalWrite(CONTROL_A, LOW); // motor off when both controlA and controlB are same level
 analogWrite(CONTROL_B, 0);
